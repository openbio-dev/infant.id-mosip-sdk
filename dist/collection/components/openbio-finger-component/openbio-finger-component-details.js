...
export class OpenbioFingerComponent {
    ...
    async setI18nParameters(locale) {
        ...
        this.fingerNames = [this.translations.RIGHT_THUMB, this.translations.RIGHT_INDEX, this.translations.RIGHT_MIDDLE, this.translations.RIGHT_RING, this.translations.RIGHT_PINKY, this.translations.LEFT_THUMB, this.translations.LEFT_INDEX, this.translations.LEFT_MIDDLE, this.translations.LEFT_RING, this.translations.LEFT_PINKY];
    ...
    executeMatch() {
        this.setLoader(true, this.translations.EXECUTING_MATCHING);
    ...
    async nfiqEvaluation(data) {
        ...
        if (data.fingersData && data.fingersData.fingerCount > 1) {
            for (const index in this.currentFingerSequence) {
                if (nfiqEvaluation && nfiqEvaluationEnabled && data.fingersData.images[index] && data.fingersData.images[index].nfiqScore > nfiqEvaluationThreshold) {
                    await this.insertPendingCapture({ fingerIndex: this.currentFingerSequence[index], captureType: this.captureType });
    ...
    async smearEvaluation(data) {
        if (this.serviceConfigs.finger.validateCaptureErrors && this.captureType === captureType.ROLLED_FINGER && data.imageError) {
            this.setLoader(false);
            ...
            const message = this.translations[data.imageError && data.imageError.code] || (data.imageError && data.imageError.text) || SMEAR_MESSAGE;
    ...
    setFlowType(deviceName, flowType) {
        const isInfantDevice = [constants.device.ETAN, constants.device.NILMA].includes(deviceName);
        const isPinchOrTenFlat = [flowTypes.FLOW_TYPE_TEN_FLAT_CAPTURES, flowTypes.FLOW_TYPE_PINCH].includes(flowType);
        if (isInfantDevice) {
            this.flowType = isPinchOrTenFlat ? flowType : flowTypes.FLOW_TYPE_TEN_FLAT_CAPTURES;
        }
        else {
            ...
            try {
                ...
                try {
                    ...
                    if (!this.singleCaptureSt) {
                        this.setFlowType(this.payload.deviceName, this.modalSettings.flowType);
                    }
                ...
                this.ws.componentSocket.addEventListener("message", (event) => {
                    ...
                    if (data.action === "session-data") {
                        ...
                        const flowType = this.backendSession.flowType !== null && this.backendSession.flowType !== undefined && this.backendSession.flowType !== "?" ? this.backendSession.flowType : this.modalSettings.flowType;
                        this.setFlowType(data.deviceName, flowType);
                        if (this.useOpenbioMatcherSt && this.singleCaptureSt) {
                            this.getPersonInfo();
                        }
                        if (this.singleCaptureSt) {
                ...
                this.ws.deviceSocket.addEventListener("message", async (event) => {
                    ...
                    if (data.status === "initialized") {
                        if (data.deviceName === constants.device.ETAN || data.deviceName === constants.device.NILMA) {
                            this.payload.deviceName = data.deviceName;
                            this.setFlowType(data.deviceName, this.modalSettings.flowType);
                            this.setCaptureType();
                            this.checkSessionData();
                        }
                        if (this.payload.processorName) {
                    ...
                    const moduleToBeUsed = ((this.payload.deviceName === constants.device.ETAN || this.payload.deviceName === constants.device.NILMA) && this.modalSettings.canUseInfant) ? "infant" : "finger";
                    if (data.module === moduleToBeUsed) {
                        ...
                        if (data.error) {
                            ...
                            if (data.fingersData.fingerCount === 0) {
                                await this.sendServiceTimeInformation(this.translations.VERIFY_FINGER_POSITION, this.translations[captureType[this.captureType]]);
                                return Swal.fire({
                                    type: 'warning',
                            ...
                            if (this.isEditing || this.doesCountMatchFlow(data.fingersData.fingerCount)) {
    ...
    async setCurrentFinger() {
        ...
        const _this = this;
        const noFingersToCapture = !this.fingersToCapture || (this.fingersToCapture && this.fingersToCapture.length === 0);
        if (noFingersToCapture && fingerSessionData.length < orderedSequence.length) {
    ...
    async saveAmputatedHand() {
        ...
        if (this.currentFingerSequence.length === 1) {
            // right hand index is always below 4
            isRightHand = this.currentFingerSequence[0] <= 4;
        }
        else if (this.currentFingerSequence.length >= 2 && this.currentFingerSequence[0] === 5 && this.currentFingerSequence[1] === 0) {
            // when the current finger sequence is [5, 0] that means the person has all the right fingers, but not the left ones, so we will set the left thumb to be edited
            isRightHand = false;
        }
        ...
        // When has more than one finger per step, do edit
        if (fingersPerStep !== 1) {
            const fingerIndexToBeEdited = isRightHand ? endIndex : 0;
            const prevFingerIndex = isRightHand ? fingerIndexToBeEdited - 1 : fingerIndexToBeEdited + 1;
            const prevCaptureType = this.fingers.find(finger => finger.fingerIndex === prevFingerIndex);
            await this.insertPendingCapture({ fingerIndex: fingerIndexToBeEdited, captureType: prevCaptureType });
            this.forceUpdate();
            this.editFinger(this, Object.assign({}, this.fingersToCapture[0]));
            this.fingersToCapture = [];
        }
        ...
        for (const index in this.currentFingerSequence) {
            ...
            if (this.anomalies[fingerIndex]) {
                if (this.checkCaptureNeed(this.anomalies[fingerIndex].id)) {
                    ...
                    anomaliesWithImage++;
                    await this.insertPendingCapture({ fingerIndex, captureType: this.captureType });
                }
            }
            else {
                Swal.fire({
    ...
    async saveFingers(fingersData) {
        ...
        for (const index in this.currentFingerSequence) {
            ...
            fingers.push({
                ...
                template: fingersData && fingersData.templates ? fingersData.templates[index] : "",
                ...
                captureType: this.captureType === captureType.ONE_FINGER_FLAT || this.captureType === captureType.TWO_FINGER_FLAT ? captureType.TWO_FINGER_FLAT : this.captureType === captureType.ROLLED_FINGER ? captureType.ROLLED_FINGER : captureType.FOUR_FINGER_FLAT,
                fingerIndex: fingerIndex,
                anomalyId: (this.anomalies && this.anomalies[fingerIndex] && this.anomalies[fingerIndex].id) || (this.anomaly && this.anomaly.id) || null,
    ...
    storeCapturedFinger(saveFingersResult) {
        ...
        this.unmatchCount = 0;
        this.repeatedCount = 0;
        if ([flowTypes.FLOW_TYPE_SEQUENCE_CONTROL_CAPTURE, flowTypes.FLOW_TYPE_TWO_FLAT_SEQUENCE_CONTROL_CAPTURE, flowTypes.FLOW_TYPE_FOUR_FLAT_SEQUENCE_CONTROL_CAPTURE].includes(this.flowType) && this.captureType === captureType.ROLLED_FINGER && this.currentFingerSequence.length > 0) {
            const currentFingerIndex = this.currentFingerSequence[0];
            const flatFinger = this.fingers.find((finger) => {
                const isFlatCaptureType = [captureType.TWO_FINGER_FLAT, captureType.ONE_FINGER_FLAT, captureType.FOUR_FINGER_FLAT].includes(finger.captureType);
                return finger.fingerIndex === currentFingerIndex && isFlatCaptureType;
            });
    ...
    getNfiqClass(fingerIndex) {
        const finger = this.fingers.find((finger) => finger.fingerIndex === fingerIndex &&
            ((flatCaptureTypes.includes(this.captureType) && flatCaptureTypes.includes(finger.captureType)) ||
                (this.captureType === captureType.ROLLED_FINGER && finger.captureType === captureType.ROLLED_FINGER)));
        if (finger) {
            const type = this.captureType === captureType.ROLLED_FINGER ? 'rolled' : 'flat';
            if (this.serviceConfigs.finger.colorizeLowNfiq || finger.nfiqScore <= this.failControl.attemptLimit[fingerIndex][type].nfiq.threshold) {
                ...
                h("div", { class: "select is-small inline is-pulled-left" },
                    h("select", { onChange: this.setAnomaly.bind(this, fingerIndex), name: "anomaly" },
                        ...
                        (this.anomalyOptions || []).map((option) => {
                            ...
                            return (h("option", { value: option.id, selected: this.anomalies && this.anomalies[fingerIndex] && this.anomalies[fingerIndex].id === option.id }, translatedOption));
                        })))));
        });
    }
            ...
            h("div", { class: "window-size" },
                ...
                this.singleCaptureSt ? (h("div", { class: "card", style: { "box-shadow": "none", "padding-bottom": "10px" } },
                    ...
                    h("ul", null,
                        ...
                        this.serviceConfigs && this.serviceConfigs.applicationType !== 'H' &&
                    ...
                    h("div", { class: "column is-one-third" },
                        h("div", { class: "device-status-container" },
                            h("h6", { class: "title is-7 has-text-left" },
                                ...
                                ": ",
                                this.deviceReady && !this.showLoader ? this.translations.READY : this.translations.NOT_LOADED)),
